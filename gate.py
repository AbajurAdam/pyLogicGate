...
class gate(pygame.sprite.Sprite):
    ...
    # self-destruction method for when the gate is deleted
    ...
    def updateState(self):  # interesting aside, I'm pretty sure this whole thing is Turing complete
        ...
        if self.updateStateDelay >= timedelay:
            self.updateStateDelay = 0
            if self.type == "or":
                self.outSocket.setState(
                    self.inSockets[0].state or self.inSockets[1].state)
            elif self.type == "and":
                self.outSocket.setState(
                    self.inSockets[0].state and self.inSockets[1].state)
            elif self.type == "xor":
                self.outSocket.setState(
                    self.inSockets[0].state ^ self.inSockets[1].state)
            elif self.type == "xnor":
                self.outSocket.setState(
                    not (self.inSockets[0].state) ^ (self.inSockets[1].state))
            elif self.type == "nor":
                self.outSocket.setState(
                    not(self.inSockets[0].state or self.inSockets[1].state))
            elif self.type == "nand":
                self.outSocket.setState(
                    not(self.inSockets[0].state and self.inSockets[1].state))
            elif self.type == "not":
