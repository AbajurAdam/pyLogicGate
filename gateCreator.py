...
class gateCreator(pygame.sprite.Sprite):
    ...
    # this function appears in all the gate-like classes, it just checks if the mouse is on the button and is called whenever theres a mousedown.
    ...
    def setType(self, gateType):
        self.type = gateType
        if self.type == "switch":
            # bulbs and switches have two images each
            self.image, self.rect = load_image(
                os.getcwd()+"/"+gateType+'0.bmp', -1)
        elif self.type == "bulb":
            self.image, self.rect = load_image(
                os.getcwd()+"/"+gateType+'0.png', -1)
        else:
            self.image, self.rect = load_image(
...
class switch(pygame.sprite.Sprite):
    ...
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = None
        # always a list, even if theres just one input -> makes the rest of the code easier.
        self.inSockets = []
        self.outSocket = None
        self.state = False
    ...
    # called when the object is deleted. Here we attempt to destroy all sockets and cables attached to the gate (this works the same way in the actual gate class)
    ...
    def push(self):  # change the switch state and load the appropriate image
        if self.outSocket.state is True:
    ...
    def update(self):
        self.screen.blit(self.image, (self.rect.topleft))
        # we always update sockets from within the gate classes' update method.
        self.outSocket.update()
...
class bulb(pygame.sprite.Sprite):
    ...
    def update(self):
        # There's only one, I jsut added this so the program wouldn't crash when there  isn't one for some reason (could also have an if len(self.inSockets), or try(...) )
